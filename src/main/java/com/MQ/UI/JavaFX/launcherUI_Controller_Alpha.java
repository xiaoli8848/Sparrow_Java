 ...
 */
public class launcherUI_Controller_Alpha {
    ...
    public void Init() {
        //launchLanguage(launcherUI_Alpha.defaultLocale);
        try {
    ...
    void launchGame(ActionEvent event) {
        if (rootDir != null && rootDir != "") {
            if (!isOnlineLaunch.isSelected()) {
                lockArgs();
                launcherUI_Alpha.launchGameOffline();
            } else {
                if (user_name.getText() != "" && password.getText() != "") {
                    lockArgs();
                    launcherUI_Alpha.launchGameOnline();
                } else {
                    //TODO 处理空账号/密码
                }
            }
    ...
    void chooseRootDir(ActionEvent event) {
        ...
        String chooserTemp = directoryChooser.showDialog(launcherUI_Alpha.primaryStage).getPath();
        if (chooserTemp != null) {
            rootDir = chooserTemp;
        }
        Init();
    ...
    void showAbout(ActionEvent event) {
        ...
        FXMLLoader a = new FXMLLoader(getClass().getClassLoader().getResource("UI/JavaFX/launcherUI_Alpha_javafx_about.fxml"));
        Parent root = null;
        try {
            root = a.load();
        } catch (IOException e) {
